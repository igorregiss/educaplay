...
if (isset($_GET['id'])) {
  ...
  $sql = "SELECT * FROM videoaula WHERE id = ?";
...
function processarUploadImagem($id) {
    if (isset($_FILES['novaImgCapa']['name']) && !empty($_FILES['novaImgCapa']['name'])) {
        $uploadDir = 'caminho/para/armazenamento/'; // Substitua pelo caminho real do armazenamento
        $uploadFile = $uploadDir . basename($_FILES['novaImgCapa']['name']);

        if (move_uploaded_file($_FILES['novaImgCapa']['tmp_name'], $uploadFile)) {
            $novoCaminhoImagem = 'caminho/para/armazenamento/' . $_FILES['novaImgCapa']['name'];

            // Atualize o banco de dados com o novo caminho da imagem
            global $conexao;
            $sql = "UPDATE videoaula SET imgCapa = ? WHERE id = ?";
            $stmt = mysqli_prepare($conexao, $sql);
            mysqli_stmt_bind_param($stmt, "si", $novoCaminhoImagem, $id);
            mysqli_stmt_execute($stmt);
        } else {
...
// Verifique se foi fornecido um ID de jogo para edição
if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // Consulta SQL para buscar os dados do jogo a ser editado usando prepared statement
    $sql = "SELECT * FROM videoaula WHERE id = ?";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Redirecione para uma página de erro ou faça algo apropriado caso o jogo não seja encontrado
        header('Location: pagina_de_erro.php');
        exit();
    }
    ...
    // Redirecione para uma página de erro ou faça algo apropriado se o ID de jogo não for fornecido
    header('Location: pagina_de_erro.php');
    exit();
}

// Processar o upload da nova imagem, se aplicável
processarUploadImagem($id);
?>
...
<body class='g-sidenav-show   bg-gray-100'>
    ...
    <main class='main-content position-relative border-radius-lg '>
        ...
        <!-- End Navbar -->
        <form id='form-edicao' action='acoes/update-videos.php' method='POST' enctype='multipart/form-data'>
        <input type='hidden' name='id' value='<?php echo $videoaula['id']; ?>'>
                                ...
                                <div class='row'>
                                            ...
                                            <select class='form-control' name='idCategoria' id='idCategoria'>
                                                ...
                                                if ($result) {
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        $selected = ($idCategoria == $videoaula['idCategoria']) ? 'selected' : '';
                                    ...
                                    <div class='col-md-6'>
                                        <div class='form-group'>
                                            ...
                                            <input class='form-control' type='text' name='titulo' id='titulo' value="<?php echo $videoaula['titulo']; ?>">
                                        </div>
                                    </div>
                                </div>
                                <div class='row'>
                                    <div class='col-md-12'>
                                        ...
                                        <input class='form-control' type='text' name='iframe' id='iframe' value="<?php echo $videoaula['iframe']; ?>">
                                        </div>
                                    </div>
                                    </div>
                                <div class='row'>
                                    <div class='col-md-6'>
                                        <div class='form-group'>
                                            ...
                                            <input class='form-control' type='file' name='novaImgCapa' id='novaImgCapa'>
                                        </div>
                                    </div>
             
                                    <div class='col-md-6'>
                                        <div class='form-group'>
                                            <label for='creditos' class='form-control-label'>Creditos</label>
                                            <input class='form-control' type='text' name='creditos' id='creditos' value="<?php echo $videoaula['creditos']; ?>">
                                        </div>
                                    </div>
                                </div>
