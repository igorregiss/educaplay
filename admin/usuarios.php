...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST['id'];
    $novoCargo = ($_POST['cargo'] == 0) ? 1 : 0; // Inverte o valor do cargo

    // Atualize o cargo do usuário
    $sql = "UPDATE usuarios SET cargo = ? WHERE id = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("ii", $novoCargo, $id);
    $stmt->execute();
    $stmt->close();
}
...
foreach ($result as $r) {
    ...
    $dtNasc = $r['dtNascimento'];
    $cargo = $r['cargo'];

    $cargoLabel = ($cargo == 1) ? "Remover admin" : "Tornar admin";
    $cargoClass = ($cargo == 1) ? "text-danger" : "text-dark";
    ...
    <tr>
        ...
        <td class='align-middle text-center'>
            <span class='text-secondary text-xs font-weight-bold'>$dtNasc</span>
        </td>
        <td class='text-center'>
            <form method='POST' action='usuarios.php'>
                <input type='hidden' name='id' value='$id'>
                <input type='hidden' name='cargo' value='$cargo'>
                <button type='submit' class='btn btn-link $cargoClass px-3 mb-0'>
                    <i class='fas fa-key text-dark me-2' aria-hidden='true'></i>$cargoLabel
                </button>
            </form>
        </td>
    </tr> 
